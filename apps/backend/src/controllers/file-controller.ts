...
// 12. get all files in a folder
const getAllFilesInFolder = async (req: Request, res: Response) => {
    const userId = req.user?.id as string
    if (!userId) {
        throw new ApiError(401, "User id not found")
    }
    ...
    try {
        ...
        console.error('Error getting all files in folder:', error);
        res.status(500).json({
            "success": false,
            "message": "Error getting all files in folder",
            "error": error instanceof Error ? error.message : String(error)
        })
    }
}
...
export { uploadFiles, getAllFiles, getFileById, deleteFilePermanently, deleteFileFromTrash, moveFileToTrash, restoreFileFromTrash, moveFileToAnotherFolder, getAllFilesInFolder, searchFilesByName, sortFiles, downloadFile };
